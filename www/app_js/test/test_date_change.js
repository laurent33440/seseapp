 ...
 //recupère les parametres passes au script appelé par :  <script src="app_js/test/mon_script.js?params=val1&val2&..."></script>
    ...
    for (var i = 0 ; i < allScripts.length ; i++) { // Pour chacun des noeuds...
    	...
    	if (currentScript.src // ... s'il a un attribut src non vide...
    			&& /test_date_change.js\?params=(.*)/.test(currentScript.src)) { // ...et que ça correspond à mon_script.js avec un paramètre 'param' derrière...
    			params = currentScript.src.match(/test_date_change.js\?params=(.*)/); // ...on capture de tout ce qu'il y a après 'param='
...
//    			alert('derriere param= : '+params[1]); // param[1] contient tout ce qu'il y a après 'param='
    ...
    var listParams_test_date_change = params[1].split('&');
    ...
    $('[type="date"]').change(function(){
           ...
           console.log(val);
           $.ajax({
               url:listParams_test_date_change[0],
               data:{       
